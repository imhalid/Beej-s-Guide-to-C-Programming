...
## <span class="header-section-number">11.1</span> Pointer Arithmetic
...
### <span class="header-section-number">11.1.1</span> Adding to Pointers
...
What happened there? C knows that `p` is a pointer to an `int`. So it knows the `sizeof` an `int`<a href="function-specifiers-alignment-specifiersoperators.html#fn84" id="fnref84" class="footnote-ref" role="doc-noteref"><sup>84</sup></a> and it knows to skip that many bytes to get to the next `int` after the first one!
...
### <span class="header-section-number">11.1.2</span> Changing Pointers
...
### <span class="header-section-number">11.1.3</span> Subtracting Pointers

You can subtract a value from a pointer to get to earlier address, as well, just like we were adding to them before.

But we can also subtract two pointers to find the difference between them, e.g. we can calculate how many `int`s there are between two `int*`s. The catch is that this only works within a single array<a href="function-specifiers-alignment-specifiersoperators.html#fn85" id="fnref85" class="footnote-ref" role="doc-noteref"><sup>85</sup></a>—if the pointers point to anything else, you get undefined behavior.
...
## <span class="header-section-number">11.2</span> Array/Pointer Equivalence
...
The one big difference is that you can *modify* a pointer to point to a different address, but you can’t do that with an array variable. In other words, you can’t assign into an array variable at all—only into individual elements of that array.

If you really want to copy one array to another, you have to use a function like `memcpy()` (or a loop) to make that happen.

### <span class="header-section-number">11.2.1</span> Array/Pointer Equivalence in Function Calls
...
## <span class="header-section-number">11.3</span> `void` Pointers
...
2.  Another function is calling a function you passed to it (a callback), and it’s passing you data. You know the type of the data, but the function calling you doesn’t. So it passes you `void*`s—’cause it doesn’t know the type—and you convert those to the type you need. The built-in [`qsort()`](https://beej.us/guide/bgclr/html/split/stdlib.html#man-qsort)<a href="function-specifiers-alignment-specifiersoperators.html#fn86" id="fnref86" class="footnote-ref" role="doc-noteref"><sup>86</sup></a> and [`bsearch()`](https://beej.us/guide/bgclr/html/split/stdlib.html#man-bsearch)<a href="function-specifiers-alignment-specifiersoperators.html#fn87" id="fnref87" class="footnote-ref" role="doc-noteref"><sup>87</sup></a> use this technique.
...
4.  You cannot use array notation on a `void*`, since it’s also a dereference, as well<a href="function-specifiers-alignment-specifiersoperators.html#fn88" id="fnref88" class="footnote-ref" role="doc-noteref"><sup>88</sup></a>.
...
And conversion is easy: you can just assign into a variable of the desired type<a href="function-specifiers-alignment-specifiersoperators.html#fn89" id="fnref89" class="footnote-ref" role="doc-noteref"><sup>89</sup></a>.
...
Now that we’ve done that, I just want to quickly point out that we can use this technique to iterate over the bytes of *any* object in C, `float`s, `struct`s, or anything!

<span id="qsort-example">Let’s</span> run one more real-world example with the built-in `qsort()` routine that can sort *anything* thanks to the magic of `void*`s.
